...
public class AccountController {
    ...
    @PostMapping("/")
    @Transactional
    public String transfer(@RequestParam String from, @RequestParam String to, @RequestParam Integer amount) throws Exception {
        Account accountFrom = this.accountRepository.findByIban(from);
        Account accountTo = this.accountRepository.findByIban(to);

        if (accountFrom.getBalance() < amount)
            throw new Exception("too little money");
